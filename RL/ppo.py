...
class PPO:
	...
	def learn(self, total_timesteps):
		...
		while t_so_far < total_timesteps:                                                                       # ALG STEP 2
			...
			batch_obs, batch_acts, batch_log_probs, batch_rtgs, batch_lens = self.rollout_marl()
			...
			avg_ep_rews.append(np.mean([np.sum(ep_rews) for ep_rews in self.logger['batch_rews']]))
	...
	def _init_hyperparameters(self, hyperparameters):
		...
		self.n_updates_per_iteration = 5                # Number of times to update actor/critic per iteration (epochs)
		...
		self.save_freq = 10                             # How often we save the NNs in number of iterations
