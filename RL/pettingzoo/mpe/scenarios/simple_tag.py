...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world, np_random):
        ...
        # Set custom obstacle positions
        pos_list = [np.array([-1.2, -0.6]), np.array([0.1, -1.1]), np.array([-0.3, 0.4]), np.array([0.9, 0.75]),
                    np.array([-0.9, 1.2]), np.array([-0.1, 1.3]), np.array([-1.2, 0.0]), np.array([1.3, 0.0])]

        for i, landmark in enumerate(world.landmarks):
            if not landmark.boundary:
                landmark.state.p_pos = pos_list[i]
